  ...
  return (
    <>
      <div className=" bg-zinc-900 px-8 md:px-12 py-24 flex flex-col md:flex-row gap-4 text-white min-h-[90vh]">
        {loader && (
          <div className=" w-full flex justify-center items-center p-10 h-96 ">
            <Hourglass
              visible={loader}
              height="90"
              width="90"
          ...
          <>
            <div className="    w-full md:w-3/6 flex justify-around items-center">
              <div className="bg-zinc-800 p-4 rounded flex flex-col md:flex-row justify-evenly w-full">
                ...
                {isLogin && role === "user" && (
                  <div className=" flex md:flex-col md:justify-normal justify-between gap-8 text-4xl px-4 md:mt-0 mt-4">
                    <button
                      className=" text-red-500 hover:text-red-600 hover:bg-zinc-200 transition-all duration-300 outline-none bg-zinc-100 rounded-full p-3 text-center"
                      onClick={handleFavourites}
                    >
                      <FaHeart />
                    </button>
                    <button
                      className=" text-zinc-100 hover:text-zinc-300 hover:bg-blue-600 transition-all duration-300 outline-none bg-blue-500 rounded-full p-3"
                      onClick={handleCart}
                    >
                      <FaShoppingCart />
                    </button>
                ...
                {isLogin && role === "admin" && (
                  <div className=" flex md:flex-col md:justify-normal justify-between gap-8 text-4xl px-4 md:mt-0 mt-4">
                    <button
                      className="  text-zinc-100 hover:text-zinc-300 hover:bg-blue-700 transition-all duration-300 outline-none bg-blue-500 rounded-full p-3 text-center"
                      onClick={() => navigate(`/update-book/${id}`)}
                    >
                      <MdEdit />
                    </button>
                    <button
                      className=" text-white-100 hover:text-zinc-200   transition-all duration-300 outline-none bg-red-500 rounded-full p-3 hover:bg-red-700"
